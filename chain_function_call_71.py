...
# shot
# Functions: 5
# First calls get_coordinates() to get lat and lon of the city and then calls the get_air_data() with these lat and lon to get current air quality data and then plot_air_data() and then size_plot_file() and then finds floor ceiling value of the file size value by size_floor_ceiling()
...
import math  # Import math module for floor and ceiling functions
...
def size_floor_ceiling(file_size_kb: float) -> dict:
    """Compute the floor and ceiling of the file size in kilobytes."""
    try:
        floor_value = math.floor(file_size_kb)
        ceiling_value = math.ceil(file_size_kb)
        return {"floor": floor_value, "ceiling": ceiling_value}
    except Exception as e:
        return {"error": str(e)}
...
size_floor_ceiling_schema = {
    "name": "size_floor_ceiling",
    "description": "Compute the floor and ceiling of the file size in kilobytes.",
    "parameters": {
        "type": "object",
        "properties": {
            "file_size_kb": {
...
routes = [
    Route(
        ...
        utterances=[
            "Plot air quality of London, measure the plot file size, and compute its floor and ceiling",
            "Can you plot air quality data of Delhi, tell me the file size, and find its floor and ceiling values?",
            "Kindly plot air quality data of Kolkata, measure the size of the plot file, and compute floor and ceiling",
        ],
        dynamic=True,
        functions=[get_coordinates, get_air_data, plot_air_data, size_plot_file, size_floor_ceiling],
        function_schemas=[get_coordinates_schema, get_air_data_schema, plot_air_data_schema, size_plot_file_schema, size_floor_ceiling_schema]
    )
]
...
def create_multi_shot_prompt(prompt: str, route: Route) -> str:
    examples = """
Example 1:
User: Plot air quality of Sydney, measure the plot file size, and compute its floor and ceiling values
Assistant:
...
Function: size_floor_ceiling
Arguments: {"file_size_kb": "$size_plot_file.file_size_kb"}
Result: {"floor": 102, "ceiling": 103}

Example 2:
User: Plot air quality of New York, tell me the file size, and compute floor and ceiling
Assistant:
Function: get_coordinates
...
Function: size_floor_ceiling
Arguments: {"file_size_kb": "$size_plot_file.file_size_kb"}
Result: {"floor": 98, "ceiling": 99}

Example 3:
User: Can you plot air quality of Delhi, measure the file size, and find floor and ceiling values?
Assistant:
Function: get_coordinates
        ...
        if function and arguments is not None:
            ...
            for arg_name, arg_value in arguments.items():
                if isinstance(arg_value, str) and arg_value.startswith("$"):
                    ...
                    # Handle nested keys for dictionaries
                    var_parts = arg_value[1:].split('.')
                    var_name = var_parts[0]
                    if var_name in context:
                        value = context[var_name]
                        # Traverse nested keys if any
                        for key in var_parts[1:]:
