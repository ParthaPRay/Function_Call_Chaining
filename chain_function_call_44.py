...
# shot
# Functions: 2
# First calls get_coordinates() to get lat and lon of the city and then calls the get_air_data() with these lat and lon to get current air quality data
...
# Get air quality: curl -X POST http://localhost:5000/process_prompt -H "Content-Type: application/json" -d '{"prompt": "Get air quality of Los Angeles."}'
...
routes = [
    Route(
        ...
        utterances=[
            "Get air quality of Los Angeles",
            "What is current air quality of Mumbai?",
            "Find air quality data of Kolkata"
        ],
        dynamic=True,
        functions=[get_coordinates, get_air_data],
        function_schemas=[get_coordinates_schema, get_air_data_schema]
    )
]
...
def create_multi_shot_prompt(prompt: str, route: Route) -> str:
    examples = """
Example 1:
User: Get air quality of Los Angeles
Assistant:
Function: get_coordinates
Arguments: {"city_name": "Los Angeles"}
Result: {"latitude": 34.0522, "longitude": -118.2437}
...
Example 2:
User: Find air quality data of Tokyo
Assistant:
Function: get_coordinates
Arguments: {"city_name": "Tokyo"}
Result: {"latitude": 35.6895, "longitude": 139.6917}
Function: get_air_data
Arguments: {"lat": 35.6895, "lon": 139.6917}
Result: {
  "coord": {
    "lon": 139.6917,
    "lat": 35.6895
